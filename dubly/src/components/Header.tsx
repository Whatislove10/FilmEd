      ...
      <div className="container flex h-14 max-w-screen-2xl items-center">
        ...
        <div className="mr-4 flex">
          <Link to="/" className="mr-6 flex items-center space-x-2">
            ...
            <span className="font-bold">Filmit</span>
          </Link>
        </div>
              ...
              {navLinks.map((link) => (
                <NavigationMenuItem key={link.href}>
                  ...
                  <NavigationMenuLink asChild>
                    <Link to={link.href} className="group inline-flex h-10 w-max items-center justify-center rounded-md bg-transparent px-4 py-2 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50 gap-2">
                      {link.icon}
                      {link.label}
                    </Link>
                  </NavigationMenuLink>
